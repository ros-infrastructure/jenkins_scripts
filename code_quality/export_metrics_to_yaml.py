...
class ExportYAML:
	...
	helper = subprocess.Popen(('%s/jenkins_scripts/code_quality/cloc.pl %s --not-match-d=%s/build --yaml --out %s'%(os.environ['WORKSPACE'],self.path, self.path, filename)).split(' '),env=os.environ)
        helper.communicate()
...
def _load_code_quality_file(filename, name, type_='package'):
    ...
    if not data:
        raise UtilException("Unable to retrieve code quality data from (%s). Auto-generated documentation may need to regenerate" % filename)
    return data
