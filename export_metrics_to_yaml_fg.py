...
def get_options(required, optional):
    ...
    # check which modules to add
    if options.distro == 'electric':
	import roslib; roslib.load_manifest("job_generation")
    #else:
...
class ExportYAML:
	...
	# Parse distro file and get uri
        if options.distro == 'electric':
	    rosdistro_obj = rosdistro.Distro(get_rosdistro_file(self.distro))
	    vcs = rosdistro_obj.stacks[self.stack].vcs_config
        else:
	    distro_obj = rospkg.distro.load_distro(rospkg.distro.distro_uri(options.distro))
	    vcs = distro_obj.stacks[self.stack].vcs_config
